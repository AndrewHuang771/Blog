from flask import render_template, flash, redirect, url_for, request, abort, send_from_directory, Blueprint
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash( user.password, form.password.data ):
            ...
            flash(f'Login Unsucessful.', 'className')
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Your password has been updated', 'className')
        return redirect(url_for('users.login'))
